...
from api_generator.posts_generator import create_paginated_posts_api
...
async def run_scraper(args):
    ...
    document_types = ["encyclicals", "homilies", "angelus", "audiences", "letters", "messages", "speeches"]
    ...
    doc_index_scraper = VaticanDocumentsIndexScraper(
        ...
        resume=args.resume or args.skip_documents_with_exists,
    )
    ...
    # Step 4: Generate posts API
    logger.info("Generating posts API...")
    try:
        create_paginated_posts_api(args.output_dir)
        logger.info("Posts API generated successfully!")
    except Exception as e:
        logger.error(f"Failed to generate posts API: {e}")
        raise
